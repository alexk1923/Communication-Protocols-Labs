				...
				if (i == sockfd) {
					...
					for(int j = 0; j <= fdmax; j++) {
						if(j != newsockfd && j!=sockfd && FD_ISSET(j, &read_fds)) {
							char msg[1024];
							sprintf(msg, "Clientul %d a fost adaugat", newsockfd);
							// printf(" j = %d", j);
					...
					for(int j = 0; j <= fdmax; j++) {
						...
						int dest_client = atoi(buffer);
						printf("Trebuie sa trimitem mesaj pe %d", dest_client);
						memmove(buffer, buffer + 2, strlen(buffer));
						send(dest_client, buffer, n, 0);
